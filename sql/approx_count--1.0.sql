...
CREATE TABLE IF NOT EXISTS @extschema@.metrics (
...
CREATE OR REPLACE FUNCTION @extschema@.approx_count_info(
...
BEGIN
    ...
    IF _relkind = 'I' THEN
        RAISE EXCEPTION '@extschema@.approx_count does not support partitioned indexes (%.% has relkind I); target a leaf partition''s index instead',
            _schemaname, _tablename
            USING ERRCODE = 'wrong_object_type';
    END IF;

    IF _relkind NOT IN ('r', 'p', 'm', 'f', 'i') THEN
        RAISE EXCEPTION '@extschema@.approx_count only supports tables, partitioned tables, materialized views, foreign tables, and indexes; %.% has relkind %',
    ...
    THEN
        INSERT INTO @extschema@.metrics AS m (
            relid,
            schemaname,
            tablename,
            total_calls_served,
...
CREATE OR REPLACE FUNCTION @extschema@.approx_count(
    target_table regclass,
    max_stale interval DEFAULT NULL,
    debug_mode boolean DEFAULT false
)
...
BEGIN
    SELECT estimated_rows
      INTO _rows
      FROM @extschema@.approx_count_info(target_table, max_stale, debug_mode);
    RETURN _rows;
END;
$$;

CREATE OR REPLACE FUNCTION @extschema@.tune_thresholds()
RETURNS void
LANGUAGE plpgsql
SET search_path = pg_catalog, pg_temp
AS $$
...
BEGIN
    ...
    DELETE FROM @extschema@.metrics AS m
...
CREATE OR REPLACE VIEW @extschema@.dashboard AS
...
REVOKE EXECUTE ON FUNCTION @extschema@.approx_count_info(regclass, interval, boolean) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION @extschema@.approx_count(regclass, interval, boolean) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION @extschema@.tune_thresholds() FROM PUBLIC;
...
COMMENT ON TABLE @extschema@.metrics IS
...
COMMENT ON COLUMN @extschema@.metrics.relid IS 'OID of the relation this ledger row describes (primary key).';
COMMENT ON COLUMN @extschema@.metrics.schemaname IS 'Schema of the relation as of the most recent call (denormalized for display).';
COMMENT ON COLUMN @extschema@.metrics.tablename IS 'Name of the relation as of the most recent call (denormalized for display).';
COMMENT ON COLUMN @extschema@.metrics.total_calls_served IS 'Count of approx_count() calls recorded for this relation (subject to approx_count.sample_rate; not recorded in read-only/standby contexts).';
COMMENT ON COLUMN @extschema@.metrics.total_analyzes_executed IS 'Count of recorded calls that ran a foreground ANALYZE.';
COMMENT ON COLUMN @extschema@.metrics.avg_full_count_duration IS 'Operator-supplied estimate of an exact COUNT(*) duration for this relation; used only to derive total_time_saved. Defaults to 15 seconds.';
COMMENT ON COLUMN @extschema@.metrics.total_time_saved IS 'Estimated cumulative time avoided, accrued only on calls that did not run ANALYZE. An estimate, not a measurement.';
COMMENT ON COLUMN @extschema@.metrics.total_analyze_duration IS 'Measured cumulative wall-clock time spent in foreground ANALYZE for this relation.';
COMMENT ON COLUMN @extschema@.metrics.last_analyze_duration IS 'Measured duration of the most recent foreground ANALYZE for this relation.';
COMMENT ON COLUMN @extschema@.metrics.max_analyze_duration IS 'Measured longest foreground ANALYZE observed for this relation.';
COMMENT ON COLUMN @extschema@.metrics.optimal_stale_interval IS 'Fallback stale threshold used when approx_count() is called with NULL max_stale; maintained by tune_thresholds().';

COMMENT ON FUNCTION @extschema@.approx_count_info(regclass, interval, boolean) IS
...
COMMENT ON FUNCTION @extschema@.approx_count(regclass, interval, boolean) IS
...
COMMENT ON FUNCTION @extschema@.tune_thresholds() IS
...
COMMENT ON VIEW @extschema@.dashboard IS
